...
agt<-system(paste("/apps/BCFTOOLS/1.9/bin/bcftools view -Ou -M2 -r chr21:",intvl[1],"-",intvl[2]," /scratch/devel/mkuhlwilm/paf/snpad/output/paf_chr21AF.vcf.gz | /apps/BCFTOOLS/1.9/bin/bcftools query -u -f '%POS %REF %ALT [%GT,] [%DP,] [%GQ,] [%AD-]\n' -",sep=""),intern=T)
if (length(agt)>0) {  
    ...
    gtQ<-GQ    
    save(nfo,gtQ,file=paste("/scratch/devel/mkuhlwilm/paf/gt3_data_",yy,".Robject",sep=""))
    ...
    save(gtAQ,nfo2,file=paste("/scratch/devel/mkuhlwilm/paf/gt3_alldata_",yy,".Robject",sep=""))
    }
