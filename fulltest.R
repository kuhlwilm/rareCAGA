...
## input: vcf file (bcftools suitable, "chr21" format)
...
load(file="/scratch/devel/mkuhlwilm/paf/snpad/aprivat/spatial_stuff")
yvec<-c(15:49)
...
alovr<-list()
save(alovr,file=paste("/scratch/devel/mkuhlwilm/paf/snpad/aprivat/",labl,sep=""))
...
for (yy in yvec) {
  ...
  # load genotypes and perform basic filtering
    agt<-system(paste("/apps/BCFTOOLS/1.9/bin/bcftools view -Ou -M2 -R /scratch/devel/mkuhlwilm/paf/snpad/aprivat/derisites/derived_9sites",yy,".bed ",infile," | /apps/BCFTOOLS/1.9/bin/bcftools query -u -f '%POS %REF %ALT [%GT,] [%DP,] [%GQ,] [%AD-]\n' -",sep=""),intern=T)
    print("loaded")
...
print("full table")
load(file=paste("/scratch/devel/mkuhlwilm/paf/snpad/aprivat/",labl,sep=""))
a1<-alovr[[15]]
for (i in 16:49) {
...
save(tmpa,ovrlai,a1,file=paste("/scratch/devel/mkuhlwilm/paf/snpad/aprivat/R1_",labl,sep=""))

## check the best fit
tst<-tmpa
tsn<-snams
prs<-list()
for (i in 1:ncol(tst)) { ts<-order(tst[,i],decreasing=T);  prs[[i]]<-c(tsn[i],coln[ts][1:3],tst[,i][ts][1:11]) }
...
# load the data
load(file="/scratch/devel/mkuhlwilm/paf/snpad/aprivat/spatial_stuff")
load(paste("/scratch/devel/mkuhlwilm/paf/snpad/aprivat/R1_",labl,sep=""))
...
pdf(paste("/scratch/devel/mkuhlwilm/paf/snpad/aprivat/map_",labl,".pdf",sep=""),8,20)
for (l in (1:pm))  { 
  print(l)
  print(tmap_arrange(allT3[[l]],ncol=1,nrow=10)) }
dev.off()

save(allT3,allTs,file=paste("/scratch/devel/mkuhlwilm/paf/snpad/aprivat/maptab_",labl,sep=""))
