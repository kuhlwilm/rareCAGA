...
module load GEOS/3.8.1 UDUNITS/2.2.26 R/3.5.0
module load PROJ/7.0.1
module load GDAL/2.4.2
LIBRARY_PATH=/apps/GCC/6.3.0/lib64:$LIBRARY_PATH
R --vanilla
library(gstat)
library(sp)
library(raster)
...
load(file=paste("/scratch/devel/mkuhlwilm/paf/snpad/coln",sep=""))
koord<-list()
...
## chimps area
area.c <- readOGR("/scratch/devel/mkuhlwilm/paf/snpad/mp//Pan_trogl_range_2016.shp")
...
save(grd,crs,fgrd,P,koord,koord3,koord2,koord4,africa,ext,coln,snam4,file=paste("/scratch/devel/mkuhlwilm/paf/snpad/aprivat/spatial_stuff",sep=""))
